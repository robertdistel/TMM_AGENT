...
constexpr uint16_t hash16(uint32_t x)
{
	return uint16_t((x &0xffff0000)>>16 & (x &0x0000ffff));
}

class Perfmon
{
public:
	Perfmon(uint16_t idx, const char * name);
	~Perfmon();
	static void dump(const char* thread_name);
private:
	uint8_t idx;
};

class PerfmonMainThread
{
public:
	PerfmonMainThread(uint16_t idx, const char * name_):name(name_)
	{
		perf_ctr= new Perfmon(idx,name_);
	}
